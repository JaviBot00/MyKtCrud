...
const val DATABASE_NAME = "DBUsers.db"
...
class SQLiteManager(context: Context) :
    ...
    override fun onCreate(db: SQLiteDatabase) {
        db.execSQL(
            "CREATE TABLE " + DB_TUsers + " ( " +
                    TUsers_USER[0] + " VARCHAR(30) PRIMARY KEY," +
                    TUsers_PASSWORD[0] + " VARCHAR(30) NOT NULL, " +
                    TUsers_BIRTHDATE[0] + " VARCHAR(10) NOT NULL, " +
                    TUsers_IMGPROFILE[0] + " BLOB, " +
                    TUsers_ROLE[0] + " VRCHAR(10) NOT NULL) "
        )
